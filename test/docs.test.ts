...
describe('user and maintainer documentation', () => {
  ...
  it('names every installer and gives safe unsigned-app guidance', async () => {
    ...
    expect(windows).toContain('DeepSeek-Harness-Desktop-0.1.1-Windows-x64.exe')
    expect(windows).toContain('SmartScreen')
    expect(macos).toContain('DeepSeek-Harness-Desktop-0.1.1-macOS-arm64.dmg')
    expect(macos).toContain('DeepSeek-Harness-Desktop-0.1.1-macOS-x64.dmg')
  ...
  it('includes licensing, security, and release-maintainer policies', async () => {
    ...
    expect(releasing).toContain('v0.1.1')
