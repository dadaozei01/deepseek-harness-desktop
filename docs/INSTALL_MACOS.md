...
- Apple Silicon（M1/M2/M3/M4 及后续）：`DeepSeek-Harness-Desktop-0.1.0-macOS-arm64.dmg`
...
```bash
shasum -a 256 DeepSeek-Harness-Desktop-0.1.0-macOS-arm64.dmg
```
...
首个版本没有 Apple Developer ID 签名。首次启动时，在 Finder 的“应用程序”中找到应用，右键（right-click）选择“打开”，再次确认“打开”。也可以在系统设置的“隐私与安全性”中核对应用后允许打开。不要通过关闭 Gatekeeper 或批量移除隔离属性来绕过系统保护。
